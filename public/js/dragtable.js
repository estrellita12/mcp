...
dragtable = {
    ...
    findUp: function (elt, tag) {
        do {
            if (elt.nodeName && elt.nodeName.search(tag) != -1) return elt;
    ...
    absolutePosition: function (elt, stopAtRelative) {
        var ex = 0,
            ey = 0;
        do {
            var curStyle = dragtable.browser.isIE
                ? elt.currentStyle
                : window.getComputedStyle(elt, "");
            var supportFixed = !(
                dragtable.browser.isIE && dragtable.browser.version < 7
            );
    ...
    dragStart: function (event, id) {
        ...
        if (browser.isIE) dragObj.origNode = window.event.srcElement;
        else dragObj.origNode = event.target;
        ...
        if (table.tHead) {
            new_elt.appendChild(
                copySectionColumn(table.tHead, dragObj.startCol)
            );
        }
        ...
        if (table.tFoot) {
            new_elt.appendChild(
                copySectionColumn(table.tFoot, dragObj.startCol)
            );
        }
