    ...
    <div class="cont_wrap">
        <div class="rhead01_wrap">
            ...
            <table>
                ...
                <tr>
                    <th scope="row">게시판</th>
                    <td colspan="2"><?=$this->boardRow['bo_name']?></td>
                    <td class="tar">
                        <?php if( $this->row['user_id']==$_SESSION['user_id'] ){ ?>
                        <a href="/Mypage/postModify/<?=$this->row['bopo_id']?>?board=<?=$_REQUEST['board']?>&returnUrl=<?=urlencode($this->returnUrl)?>" class="btn_white btn_small">
                            <img src="/public/img/icon/edit.png">수정
                        </a>
                        <a href="/Mypage/removePost/<?=$this->row['bopo_id']?>?board=<?=$_REQUEST['board']?>&returnUrl=<?=urlencode($this->returnUrl)?>" class="btn_small btn_white" onclick="return confirm('해당 게시글을 삭제 하시겠습니까?\n삭제 처리된  데이터는 복구 불가능합니다.')">
                    ...
                    <td colspan="3">
                        <div class="comment_wrap">
                            <ul class="comment_list_wrap">
                                ...
                                <div class="comment_item_input tar"><span class="add_comment btn_ssmall btn_white" >댓글</span></div>
                                <?php } ?>
                                </li>
                                <li class="comment_input_item reply marl<?=($row['boco_depth']+1)*20?> dn">
        ...
        <div class="confirm_wrap">
            ...
            <a href="/Mypage/postAnswer/<?=$this->row['bopo_id']?>?board=<?=$_REQUEST['board']?>&returnUrl=<?=urlencode($this->returnUrl)?>" class="btn_gray btn_large">
                답글
            </a>
            <?php } ?>
