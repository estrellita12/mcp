...
$excel->setCellValue($pre.'1', '총판매금액');
...
foreach($this->row as $row) {
    ...
    $excel->setCellValue($char++.$i, $row['od_amount']);
    $excel->getStyle("A$i:".$pre.$i)->applyFromArray($tdStyle);
}

$excel->getStyle('B:D')->getNumberFormat()->setFormatCode('#,##0');
...
$filename = '주문_통계_'.date("Ymd", _SERVER_TIME) ;
