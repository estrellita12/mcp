...
{
    $char = 'A';
    $excel->setCellValue($char++.$i, $this->seller->getName($row['seller_id']))
        ->setCellValue($char++.$i, $row['seller_id'] )
        ->setCellValue($char++.$i, $row['pay_time'])
        ->setCellValue($char++.$i, conv_dan($row['status']))
...
$pt_name = $this->seller->getName($_REQUEST['scPT']);
