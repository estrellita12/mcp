<section class="contents">
    ...
    <div class="cont_wrap">
        <form name="frmMypage" action="/Mypage/setLayout/<?=$this->my['pt_id']?>" method="post">
            <div class="chead02_wrap">
                ...
                <table>
                    ...
                    <thead>
                        <tr>
                            <th scope="col">타이틀</th>
                            <th scope="col">더보기 URL</th>
                            <th scope="col">타입</th>
                            <th scope="col">정적</th>
                            <th scope="col">디자인</th>
                            <th scope="col">(PC)출력갯수</th>
                            <th scope="col">(모바일)출력갯수</th>
                            <th scope="col">관리</th>
                        </tr>
                    </thead>
                    <tbody id="main_sortable" class="main_layout" style="height:50px;">
                    <?php foreach($this->my['shop_main_layout'] as $mainLayout){ ?>
                    <tr class="layout">
                        <td>
                        ...
                        <td><button type="button" class="btn_small btn_gray" onclick="trDel(this)">삭제</button></td>
                    </tr>
                    <?php } ?>
                    </tbody>
                </table>
                <div class="tar mart10"><button type="button" onclick="mainLayoutAdd()" class="btn btn_small btn_red">추가</button></div>
            </div>
            <div class="confirm_wrap">
                <?php if($this->my['shop_main_layout_mode']=="modify"){ ?>
                <a href="/Mypage/resetLayout/<?=$this->my['pt_id']?>" class="btn_large btn_gray">삭제(본사 설정 반영)</a>
                <?php } ?>
...
function apiTypeList(obj){
    ...
    switch(val){
        case "goods":
            ...
            str += "<option value='col=orderQty&colby=desc&rpp=12' "+(col=='col=orderQty&colby=desc&rpp=12'?"selected":"")+">인기상품</option>";
            str += "<option value='col=regDate&colby=desc&rpp=12' "+(col=='col=regDate&colby=desc&rpp=12'?"selected":"")+">신상품</option>";
