...
class OrderController extends Controller{
    ...
    public function list(){
        ...
        if( !empty($this->param['ident']) ) $_REQUEST['state'] = $this->param['ident'];
   ...
   public function list29(){
    ...
    function descPopup(){
        ...
        $this->row = $this->order->get($this->col,array("od_id"=>$this->param['ident']));
        $this->row['od_goods_info'] = json_decode($this->row['od_goods_info'],true);
        $this->cs = new \application\models\CustomerServiceModel();
        $_REQUEST['odId'] = $this->param['ident'];
    }

    function descListPopup(){
        $this->header='head'; $this->footer=false;
        $_REQUEST['no'] = $this->param['ident'];
        $this->col = "od_no,concat(od_id) as od_id_li, od_dt, pt_id, sum(od_qty) as od_qty, sum(od_goods_price) as od_goods_price, sum(od_use_point) as od_use_point, sum(od_use_coupon) as od_use_coupon, sum(od_delivery_charge+od_delivery_charge_dosan) as od_delivery_charge, sum(od_amount) as od_amount, sum(od_cancel_amount) as od_cancel_amount, sum(od_return_amount) as od_return_amount";
        $this->row = $this->order->getSum($this->col);
        $this->col = "*";
        $this->cnt = $this->order->getCnt();
    }
    ...
    function add(){
        $partner = new \application\models\PartnerModel();
        $pt = $partner->get("shop_pg_mid,shop_pg_service,pt_grade",array("pt_id"=>$_POST['shop']));
        if( !empty($pt['shop_pg_mid']) ){
            ...
            $df = $default->get("shop_pg_mid,shop_pg_service",array("df_id"=>"1"));
    ...
    public function analysisExcel(){
        ...
        if( empty($_REQUEST['end']) ) $_REQUEST['end'] = _DATE_YMD;
        $col = "dc_dt, count(distinct od_id) as od_cnt ";
        $this->row = $analysisModel->getAnalysis($col," ( select * from web_order where pt_id='{$_SESSION['user_id']}' ) ","od_dt");
    }
