        ...
        <div class="cont_wrap">
            <div class="chead01_wrap">
                ...
                <table>
                    ...
                    <tbody>
                    <?php foreach($this->row as $row){   $gs = json_decode($row['od_goods_info'],true);  ?>
                    <tr>
                        <td><?=$row['od_dt']?></td>
                        <td><?=od_no($row['od_no'])?></td>
                        <td><?=od_id($row['od_id'])?></td>
                        <td class="dot"><?=gs_name($row['gs_id'],$gs['goodsName'])?></td>
                        <td><?=number_format($row['od_qty'])?>개</td>
                        <td><?=number_format($row['od_goods_price'])?>원</td>
                        <td><?=number_format($row['od_delivery_charge']+$row['od_delivery_charge_dosan'])?>원</td>
