            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php foreach( $GLOBALS['od_stt'] as $key=>$value ){ if($key <= 20 || $key >= 30) continue;?>
                            <span class="order_chk">
                                <label><input type="radio" name="state" value="<?=$key?>" <?=get_checked($key,get_request("state"))?>  onclick="chkListClick(this)"><?=$value['title']?></label>
                            </span>
                            <?php } ?>
                        </td>
                    </tr>
    ...
    <form action="/Order/listUpdateState" method="post">
        ...
        <div class="layout01_wrap">
            <div class="layout_inner">
                ...
                <div class="btn_wrap">
                    <span class="right_wrap">
                        <select name="state" class="w140" required>
                    ...
                    <table>
                        ...
                        <thead>
                            <tr>
                                ...
                                <th scope="col">회수 택배사</th>
                                <th scope="col">회수 송장번호</th>
                                <th scope="col">발송 택배사</th>
                                <th scope="col">발송 송장번호</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="deliveryCompany2[]" <?=$_REQUEST['state']!=22?"disabled":""?>>
                                        <?=get_frm_option("",$row['od_delivery_company2'],"선택")?>
                                        <?php foreach($this->config['cf_delivery_company'] as $key=>$dv){ ?>
                                        <?=get_frm_option($key,$row['od_delivery_company2'],$dv['company'])?>
                                        <?php } ?>
                                    </select>
                                </td>
                                <td><input type="text" name="deliveryNo2[]" value="<?=$row['od_delivery_no2']?>" class="w100p" <?=$_REQUEST['state']!=22?"disabled":""?>></td>
                                <td>
                                    <select name="deliveryCompany3[]"  <?=$_REQUEST['state']!=23?"disabled":""?>>
                                        <?=get_frm_option("",$row['od_delivery_company3'],"선택")?>
                                        <?php foreach($this->config['cf_delivery_company'] as $key=>$dv){ ?>
                                        <?=get_frm_option($key,$row['od_delivery_company3'],$dv['company'])?>
                                        <?php } ?>
                                    </select>
                                </td>
                                <td><input type="text" name="deliveryNo3[]" value="<?=$row['od_delivery_no3']?>" class="w100p" <?=$_REQUEST['state']!=23?"disabled":""?>></td>
