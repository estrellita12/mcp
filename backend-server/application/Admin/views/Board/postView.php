<section class="contents">
    ...
    <div class="cont_wrap">
        <div class="rhead01_wrap">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td class="tar">
                        <a href="/Board/postModify/<?=$this->row['bopo_id']?>?board=<?=$_REQUEST['board']?>&returnUrl=<?=urlencode($this->returnUrl)?>" class="btn_white btn_small">
                            <img src="/public/img/icon/edit.png">
                            수정
                        </a>
                        <a href="/Board/removePost/<?=$this->row['bopo_id']?>?board=<?=$_REQUEST['board']?>&returnUrl=<?=urlencode($this->returnUrl)?>" class="btn_white btn_small" onclick="return confirm('해당 게시글을 삭제 하시겠습니까?\n삭제 처리된  데이터는 복구 불가능합니다.')">
                            <img src="/public/img/icon/delete.png">
                            삭제
                        </a>
                    </td>
                </tr>
                ...
                <tr>
                    <th scope="row">옵션</th>
                    <td>
                        <label><input type="checkbox" <?=get_checked('y',$this->row['bopo_secret_yn'])?> disabled> 비밀글</label>
                        <label><input type="checkbox" <?=get_checked('1',$this->row['bopo_main_display'])?> disabled> 메인고정</label>
                    </td>
                    <th scope="row">파일</th>
                    <td>
                    ...
                    <td colspan="3">
                        <div class="comment_wrap">
                            <ul class="comment_list_wrap">
                                <?php foreach($this->comment->getDepthList($this->row['bopo_id']) as $row){ ?>
                                ...
                                <div class="comment_item_input tar"><span class="add_comment btn_ssmall" ><img src="/public/img/icon/chat.png"></span></div>
                                </li>
        ...
        <div class="confirm_wrap">
            ...
            <a href="/Board/postAnswer/<?=$this->row['bopo_id']?>?board=<?=$_REQUEST['board']?>&returnUrl=<?=urlencode($this->returnUrl)?>" class="btn_gray btn_large">
                답글
            </a>
            <?php } ?>
