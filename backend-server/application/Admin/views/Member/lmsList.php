    ...
    <form>
        <div class="layout01_wrap">
            <div class="layout_inner">
                <div class="rect_wrap">
                    <span class="cnt_wrap">
                        검색된 LMS : <b class="cnt"><?= $this->cnt ?></b> 개
                    </span>
                    <span class="rpp_wrap">
                ...
                <div class="btn_wrap">
                    <a href="/Marketing/lmsForm" class=" btn_small btn_white"> + LMS 추가</a>
                </div>
                <div class="chead01_wrap">
                    <table>
                        <colgroup>
                            ...
                            <col class="w60">   <!-- 승인 여부 -->
                            <col class="w50">   <!-- 발송 여부 -->
                            <col class="w100">   <!-- 발송 여부 -->
                            <col class="w50">   <!-- 관리 -->
                        </colgroup>
                        <thead>
                            <tr>
                                <th scope="col">번호</th>
                                <th scope="col">LMS 제목</th>
                        ...
                        <tbody>
                            <?php $i=0; foreach($this->lms->getList($this->col) as $row) {?>
                            <tr class="list<?=$i%2?>">
                                <td class="tac"><?=($i+1)+($_REQUEST['rpp']*($_REQUEST['page']-1))?></td>
                                <td class="tal dot"><a href="/Marketing/lmsModify/<?=$row['lms_id']?>"><?=$row['lms_title']?></a></td>
                                <td>
                                    <a href="#" onclick="winOpen('/Marketing/lmsPreviewPopup/<?=$row['lms_id']?>','lmsPopup','900','600','yes');">Example</a>
                                </td>
                                <td><?=pt_id($row['pt_id'],$this->pt_li[$row['pt_id']]); ?></td>
                                <td><?=$row['lms_reg_dt']?></td>
                                <td><?=$row['lms_send_dt']?></td>
                                <td><?=$GLOBALS['lms_stt'][$row['lms_stt']]?></td>
                                <td><?=img_success($row['lms_send_yn'],"y")?></td>
                                <td>
                                    <a href="#" onclick="winOpen('/Marketing/lmsLog/<?=$row['lms_id']?>','lmsser','900','600','yes');">
                                        <?=$row['lms_send_res']?>
                                    </a>
                                </td>
                                <td><a href="/Marketing/lmsModify/<?=$row['lms_id']?>" class="btn_small btn_white">수정</a></td>
                            </tr>
                            <?php $i++; } ?>
                        </tbody>
                    </table>
