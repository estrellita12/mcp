<section class="contents">
    ...
    <div class="cont_wrap">
        <form action="/Default/setLayout" method="POST" onsubmit="return frmExample()">
            ...
            <div class="confirm_wrap">
                <input type="submit" value="저장" id="btn_submit" class="btn btn_large btn_theme" accesskey="s">
            </div>
        </form>
    </div>
</section>
...
function apiTypeList(obj){
    ...
    switch(val){
        case "goods":
            ...
            str += "<option value='col=gs_order_qty&colby=desc' "+(col=='col=gs_order_qty&colby=desc'?"selected":"")+">인기상품</option>";
            str += "<option value='col=gs_reg_dt&colby=desc' "+(col=='col=gs_reg_dt&colby=desc'?"selected":"")+">신상품</option>";
        ...
        case "banner":
            ...
            <?php foreach($this->gr_li as $idx=>$title){ ?>
                str += "<option value='bn_position=<?=$idx?>' "+(col=='bn_position=<?=$idx?>'?"selected":"")+"><?=$title?></option>";
                <?php } ?>
                    str+= "</select>"
                    break;
        case "media":
