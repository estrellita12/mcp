...
foreach($this->option->getList($this->col) as $row) {
    ...
    $excel->setCellValue($char++.$i, $row['gs_id'])
        ...
        ->setCellValue($char++.$i, $GLOBALS['gs_opt_type'][$row['gs_opt_type']] )
        ->setCellValueExplicit($char++.$i ,$row['gs_opt_add_price'],   \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_NUMERIC )
