    ...
    <div class="cont_wrap">
        ...
        <form>
            <div class="list_wrap">
                ...
                <div class="btn_wrap">
                    <a href="/Setting/mailForm" class="btn_small btn_white">+ 메일 추가</a>
                </div>
                <div class="chead01_wrap" id="reload_wrap">
                    <table>
                        <colgroup>
                            <col class="w40">   <!-- 체크박스 -->
                            <col class="w200">  <!-- 메일 제목 -->
                            <col class="w100">  <!-- 미리보기 -->
                        ...
                        <tr class="list<?=$i%2?>">
                            ...
                            <td class="tal dot">
                                <a href="/Setting/mailModify/<?=$row['tp_id']?>?returnUrl=<?=urlencode($this->returnUrl)?>"><?=$row['tp_title']?></a>
                                <?=id_log($row['tp_id'],"Template");?>
                            </td>
                            <td>
                                <a href="#" onclick="winOpen('/Setting/previewPopup/<?=$row['tp_id']?>','edmPopup','900','600','yes');">Example</a>
                            </td>
                            <td><?=$row['tp_reg_dt']?></td>
                            <td><?=$row['tp_update_dt']?></td>
                            <td>
                                <a href="#" onclick="winOpen('/Setting/sendLogPopup/<?=$row['tp_id']?>','sendLog','900','600','yes');">상세 보기</a>
                            </td>
                            <td><a href="/Setting/mailModify/<?=$row['tp_id']?>?returnUrl=<?=urlencode($this->returnUrl)?>" class="btn_small btn_white">수정</a></td>
                        </tr>
                        <?php $i++; } ?>
                        </tbody>
                    </table>
